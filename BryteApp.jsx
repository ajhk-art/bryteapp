...
export default function BryteApp() {
  ...
  useEffect(() => {
    const lagret = JSON.parse(localStorage.getItem("loggHistorikk") || "[]");
    setHistorikk(lagret);

    const eksisterendeNotat = localStorage.getItem("notat");
    if (eksisterendeNotat) setLagretNotat(eksisterendeNotat);
  }, []);
  ...
  const lagreLogg = () => {
    if (!logg.trim()) return;
    const ny = { dato: new Date().toLocaleString(), tekst: logg };
    const oppdatert = [ny, ...historikk];
  ...
  const lagreNotat = () => {
    localStorage.setItem("notat", notat);
    setLagretNotat(notat);
    setNotat("");
  };
  ...
  return (
    <div style={{ padding: 20, fontFamily: "Arial, sans-serif", maxWidth: 600, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 30 }}>
        <h2 style={{ fontSize: 20 }}>🧠 Notater</h2>
        <textarea
          value={notat}
          ...
          style={{ width: "100%", padding: 10, marginBottom: 10 }}
        />
        <button onClick={lagreNotat} style={{ padding: "10px 20px", background: "#7c3aed", color: "#fff", border: "none", borderRadius: 6 }}>
          Lagre notat
        </button>

        {lagretNotat && (
          <div style={{ marginTop: 15, background: "#f3f4f6", padding: 10, borderRadius: 4 }}>
            <strong>Ditt siste notat:</strong>
            <p>{lagretNotat}</p>
          </div>
        )}
      </div>
